package com.mtgz.sc.manager.web.shiro;
...
public class UserRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        List<String> permsList = (List<String>) cacheUtils.get(Constant.PERMS_LIST + userId);
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(
        ...
        //查询用户信息
        SysUserEntity user = sysUserMapper.queryByUserName(username);
        ...
        if (Constant.SUPER_ADMIN == user.getUserId()) {
            List<SysMenuEntity> menuList = sysMenuMapper.queryList(new HashMap<String, Object>());
            permsList = new ArrayList<>(menuList.size());
            for (SysMenuEntity menu : menuList) {
                permsList.add(menu.getPerms());
            }
        } else {
            permsList = sysUserMapper.queryAllPerms(user.getUserId());
        }
        cacheUtils.put(Constant.PERMS_LIST + user.getUserId(), permsList);
